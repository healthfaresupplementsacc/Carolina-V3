 ...
 *
 * Substeps:
 *   3a  supplements + supplement_catalog → products  (consolida)
 *   3b  persons  (7 rows da spec autoritativa ITEM 1; operators
 *       usado SÓ p/ cross-check de slack_user_id)
 *   3c  shared_accounts (3) + shared_account_users (12)
 *   3d  activity_types seed (18 rows)
 *   3e  settings seed (16 chaves)
 ...
 *   --apply                executa 3a-3f em transação única.
 *                          Exige ADMIN_CONFIRMED=TRUE. Roda ICs/Vs
 *                          DENTRO da tx — qualquer IC falha → ROLLBACK.
 *   --drop-legacy-supplements
 *                          dropa supplement_catalog + supplements.
 *                          Exige ADMIN_CONFIRMED=TRUE + products já
 *                          populada (≥13). Invocação SEPARADA, só após
 *                          PAUSA (b) liberada pelo Bruno.
 *
 * Pré-req do --apply: 001_v3_initial.sql já rodou (tabelas V3 existem).
 * Migração é one-shot: --apply recusa se persons já tiver linhas.
 *
 * Princípio #22: nenhum nome literal é lógica — os nomes abaixo são
 * DADOS de seed (a spec ITEM 1 confirmada pelo Bruno). Princípio #23:
 * identifies_as[] é hint pro LLM, nunca regex.
 *
...
async function tableExists(db, name) {
  const r = await db.query("SELECT to_regclass('public.' || $1) AS reg", [name]);
...
async function audit(db, action, after) {
  await db.query(
    `INSERT INTO audit_log (actor_type, actor_person_id, action, target_type, after_data)
...
async function dryRun(db) {
  ...
  // 3b — cross-check operators p/ os slack_user_id da spec
  for (const p of PERSONS_SEED.filter((x) => x.slack_user_id)) {
    ...
    if (!r.rows.length) {
      ...
      out.warnings.push(`cross-check: ${p.slack_user_id} → operators.name="${r.rows[0].name}" vs spec "${p.display_name}" (esperado — ITEM 1 ganha, princípio #22)`);
    }
  }
  ...
  // 3f — learning tables: se existirem, têm que estar vazias
  out.steps['3f'] = {};
  for (const t of LEARNING_TABLES) {
    out.steps['3f'][t] = (await tableExists(db, t)) ? await count(db, `SELECT COUNT(*) c FROM ${t}`) : 'tabela ainda não existe';
  }
  ...
  out.ic = {
    'IC-1 owner/manager em shared_account_users (esp. 0)': 0, // só os 4 operators entram
  ...
  // IC-5 — counts legados REAIS (essas tabelas existem)
...
// ─────────────────────────────────────────────────────────────
// APPLY — executa 3a-3f em transação; ICs/Vs dentro da tx
// ─────────────────────────────────────────────────────────────
...
async function apply(db) {
  ...
  if (await count(db, 'SELECT COUNT(*) c FROM persons') > 0) {
    throw new Error('persons já tem linhas — migração é one-shot. Restaure do backup antes de re-rodar.');
  }
  ...
  try {
    ...
    for (const p of plan.products) {
      await db.query('INSERT INTO products (canonical_name, aliases) VALUES ($1, $2::text[])',
        [p.canonical_name, p.aliases]);
      done.products++;
    }
    await audit(db, 'v3_migration.3a_products', { count: done.products, consolidated: plan.consolidated });

    // 3b — persons (id por display_name p/ resolver FKs)
    const personId = {};
    for (const p of PERSONS_SEED) {
      const r = await db.query(
        `INSERT INTO persons (display_name, role, slack_user_id, slack_dm_id, active)
    ...
    // 3c — shared_accounts + shared_account_users
    for (const s of SHARED_ACCOUNTS_SEED) {
      await db.query(
        `INSERT INTO shared_accounts (slack_user_id, primary_owner_id, slack_dm_id, description)
         VALUES ($1, $2, $3, $4)`,
      ...
      for (const u of SHARED_USERS_SEED) {
        await db.query(
          `INSERT INTO shared_account_users (shared_account_id, person_id, identifies_as)
           VALUES ($1, $2, $3::text[])`,
    ...
    for (const a of ACTIVITY_TYPES_SEED) {
      await db.query(
        `INSERT INTO activity_types (slug, display_name, category, requires_product, emoji, color)
         VALUES ($1, $2, $3, $4, $5, $6)`,
    ...
    for (const t of LEARNING_TABLES) {
      const c = await count(db, `SELECT COUNT(*) c FROM ${t}`);
      if (c !== 0) throw new Error(`3f: ${t} deveria estar vazia, achei ${c} linhas`);
    }

    // ICs / Vs REAIS dentro da tx
    const ic = {
      'IC-1': await count(db,
        `SELECT COUNT(*) c FROM shared_account_users sau
         JOIN persons p ON p.id = sau.person_id WHERE p.role IN ('owner','manager')`),
      'IC-2': await count(db,
        `SELECT COUNT(*) c FROM persons
         WHERE display_name ILIKE '%test%' OR display_name ILIKE 'adminvalidate%'
            OR display_name ILIKE '%Bia%'`),
      'IC-3': await count(db, 'SELECT COUNT(*) c FROM products'),
      'IC-4': await count(db, 'SELECT COUNT(*) c FROM activity_types'),
    };
    const v = {
      'V-1': await count(db, 'SELECT COUNT(*) c FROM persons WHERE active = true'),
      'V-2': await count(db, "SELECT COUNT(*) c FROM persons WHERE role = 'owner'"),
      'V-3': await count(db, "SELECT COUNT(*) c FROM persons WHERE role = 'manager'"),
      'V-4': await count(db, "SELECT COUNT(*) c FROM persons WHERE role = 'operator'"),
      'V-5': await count(db, 'SELECT COUNT(*) c FROM shared_accounts'),
      'V-6': await count(db, 'SELECT COUNT(*) c FROM shared_account_users'),
      'V-7': await count(db, 'SELECT COUNT(*) c FROM shared_accounts WHERE primary_owner_id IS NULL'),
      'V-8': await count(db, 'SELECT COUNT(*) c FROM persons WHERE slack_user_id IS NULL'),
    };
    const fails = [];
...
// ─────────────────────────────────────────────────────────────
// DROP LEGACY — supplement_catalog + supplements (PAUSA b)
// ─────────────────────────────────────────────────────────────

async function dropLegacySupplements(db) {
  const products = await count(db, 'SELECT COUNT(*) c FROM products');
  if (products < 13) {
    throw new Error(`recusado: products tem ${products} linhas (<13) — consolidação 3a precisa estar aplicada e válida antes do DROP.`);
  }
  await db.query('BEGIN');
  try {
    await db.query('DROP TABLE IF EXISTS supplement_catalog');
    await db.query('DROP TABLE IF EXISTS supplements');
    await audit(db, 'v3_migration.drop_legacy_supplements', { dropped: ['supplement_catalog', 'supplements'], products_at_drop: products });
    await db.query('COMMIT');
  } catch (e) { await db.query('ROLLBACK'); throw e; }
  return { dropped: ['supplement_catalog', 'supplements'], products_at_drop: products };
}

module.exports = {
  PERSONS_SEED, SHARED_ACCOUNTS_SEED, SHARED_USERS_SEED,
  ACTIVITY_TYPES_SEED, SETTINGS_SEED, LEARNING_TABLES,
  parseAliases, planSupplements, dryRun, apply, dropLegacySupplements,
};
  ...
  (async () => {
    ...
    try {
      if (mode === 'dry-run') {
        ...
        for (const [k, v] of Object.entries(r.steps['3a'])) {
          if (k === 'products') { console.log(`3a.products (${v.length}):`); v.forEach((p) => console.log(`     ${p.canonical_name}  aliases=[${p.aliases.join(', ')}]`)); }
          else if (k === 'consolidated') { console.log(`3a.consolidated (${v.length}):`); v.forEach((c) => console.log(`     ${c.canonical_name} ← ${c.sources.join(' + ')}`)); }
          else console.log(`3a.${k}: ${v}`);
        }
        ...
        for (const [k, v] of Object.entries(r.ic)) console.log(`   ${k}: ${v}`);
        console.log('\n-- Vs (hipotéticos) --');
        for (const [k, v] of Object.entries(r.v)) console.log(`   ${k}: ${v}`);
        ...
        console.log('\nsupplement_catalog/supplements NÃO foram dropados (PAUSA b — invocação separada).');
      } else {
        if (process.env.ADMIN_CONFIRMED !== 'TRUE') {
