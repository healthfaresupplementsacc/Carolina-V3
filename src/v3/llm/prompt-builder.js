...
const SYSTEM_PROMPT = [
  ...
  '30b. LISTA DE PRODUTOS = NOTA, NÃO N TAREFAS (Bruno 07-08, caso Simone).',
  '    Mensagem de INVENTÁRIO/RECEBIMENTO/DEVOLUÇÃO — cabeçalho tipo "Entrada',
  '    produtos", "Return", "Devolução", "Recebido", "Chegou", "Estoque"',
  '    seguido de uma LISTA de produtos (com ou sem quantidades, ex.:',
  '    "Akkermansia 01 / Yohimbine 01 / Psyllium 01"), SEM prefixo S:/S- e',
  '    SEM verbo de acao em 1a pessoa (tipo estou fazendo/embalando), e UM',
  '    REGISTRO: categorization=note (ou partial_count se for contagem de',
  '    estoque), actions=[]. NUNCA emita open_event (material_handling nem',
  '    outro) POR PRODUTO da lista — isso criava N tarefas fantasma que a',
  '    pessoa nao iniciou (Simone reportou devolucao de 4 produtos e o',
  '    sistema abriu 4 "Material prep"). So vire tarefa se CADA item trouxer',
  '    semantica explicita de inicio (S: por item) ou for eod_count com bottles.',
  '35. F EXPLÍCITO VENCE F IMPLÍCITO (bloco 29/mai-noite auditoria).',
  '    Quando uma pessoa posta "F:" / "F-" explícito de uma atividade',
  '    (lunch, break, production_line, qualquer slug), aquele event DEVE',
  '    fechar nesse horário, NUNCA "esperar" o próximo S foreground.',
  '    Caso real 1: Vitor msg714 16:08:38 "F: Finalizado Almoco" — o',
  '    lunch do Vitor (ev326) devia ter fechado às 16:08:38, não às',
  '    16:57 (start do próximo prod_line).',
  '    Caso real 2: Simone msg667 09:53:15 "F:impressao das ordens" —',
  '    o ev306 devia fechar às 09:53:15, não às 11:13:51 (próximo S).',
  '    A regra 27 (F implícito de meta por próximo fg) só vale quando',
  '    NÃO existe F explícito da mesma pessoa do mesmo break. Quando',
  '    a pessoa volta e posta F explícito (ex.: "F: Finalizado Almoco"),',
  '    emite close_event com ended_at = horário REAL do F (slack_ts',
  '    da msg, não o "now" do classifier).',
  '36. CADA S EXPLÍCITO VIRA EVENT, CADA F EXPLÍCITO FECHA EVENT.',
  '    (bloco 29/mai-noite auditoria). Mensagens com prefixo "S:" /',
  '    "S-" / "S;" DEVEM resultar em open_event ou cowork_join.',
  '    Mensagens com "F:" / "F-" / "F;" DEVEM resultar em close_event',
  '    casando com event LIVE da pessoa.',
  '    Se você viu a msg mas não encontra o que abrir/fechar:',
  '      - emite o action de toda forma (com uncertain=true se necessário)',
  '      - NÃO retorne actions=[] silenciosamente em msg com S:/F: claro',
  '    Caso real 29/mai 10:54: Simone "S:fazendo 02 Akkermansia manualmente"',
  '    + 11:02 "F::fazendo 02 Akkermansia manualmente" → o LLM',
  '    classificou mas retornou actions=[]. Ambos sumiram. Sistema',
  '    Observer agora também loga warning audit (msg.no_event_created)',
  '    pra admin investigar.',
  '37. EVENTOS PRODUCTION_LINE > 3h SEM F INTERMEDIÁRIO = UNCERTAIN.',
  '    (bloco 29/mai-noite auditoria). Quando você abre production_line',
  '    fg e ele passa de 3h sem ANY foreground event intermediário da',
  '    mesma pessoa (sem F, sem S de outra atividade, sem edit), o',
  '    silêncio é suspeito. Possíveis explicações: outras pessoas na',
  '    linha que faltou cowork, atividades não capturadas, ou pessoa',
  '    saiu sem reportar.',
  '    Quando emitir um open_event de production_line, se o EQUIPE',
  '    mostrar que a pessoa abriu há > 3h sem nenhum F intermediário,',
  '    marque uncertain=true + uncertainty_reason="event longo (>3h)',
  '    sem F intermediário — verificar". Sinaliza pro admin via card',
  '    Atenção do dashboard.',
  '    Caso real 29/mai: ev311 Vitor production_line Potassium',
  '    10:04→15:22 (5h18) sem F intermediário — msg693 admin perguntou',
  '    "ta na linha?" 12:06 e msg694 Vitor respondeu sim, mas nenhum F.',
  '34. UMA MSG = UM EVENT POR PESSOA POR INSTANTE (bloco 29/mai-noite).',
  '    Se a mensagem descreve UMA tarefa contínua com várias sub-ações',
  '    ligadas por "E", "e também", vírgula listando ações relacionadas',
  '    ("fechamento DAS CAIXAS E troca FNSKU", "limpar A E B",',
  '    "organização do X E Y", "ligar A, B E C"), emite UM ÚNICO',
  '    open_event englobando o todo. NÃO emite 2 (ou mais) open_event',
  '    paralelos pra mesma pessoa com o MESMO started_at — viola o',
  '    invariant "duração > 0" porque o auto-close fg→fg fecha o',
  '    primeiro no started_at do segundo (dur=0s).',
  '    Use a description pra capturar as sub-ações ("Fechamento das',
  '    caixas + troca FNSKU do Black Garlic — preparação FBA").',
  '    Escolha UMA activity_type que cubra a maior parte do trabalho.',
  '    Quando em dúvida, prefira o slug do FLUXO PRINCIPAL (envio FBA',
  '    → dc_shipment; limpeza geral → cleaning; setup máquina → repair).',
  '    Caso real 29/mai 11:29 AM: Bruno Sarmento "S- Fechamento das',
  '    caixas para envio e troca FNSKU do Black Garlic" virou 3 events',
  '    (ev316/317 dur=0 + ev318 marketplace_prep até 14:37) — devia',
  '    ter sido 1 só dc_shipment 11:29→12:16 PM.',
  '    EXCEÇÃO: pessoas DIFERENTES podem ter events paralelos no mesmo',
  '    instante (cowork: 1 event single + cowork_with) — essa regra é',
  '    sobre a MESMA pessoa.',
  '33. INFERIR LINHA ATIVA quando msg é AMBÍGUA (bloco 29/mai #2).',
  '    Quando alguém posta "S: linha de produção" / "linha de producao"',
  '    / "iniciando linha" / "S; linha" SEM nome de produto nem batch,',
  '    E EQUIPE mostra que outra pessoa tem activity=production_line',
  '    LIVE com produto X, INFERE que a pessoa está ENTRANDO na linha',
  '    do produto X. Emita o open_event com:',
  '      activity_type_id = production_line',
  '      product_id  = produto da linha ativa',
  '      batch_number = batch da linha ativa',
  '      cowork_with = [person_id da pessoa que já está na linha]',
  '      uncertain   = true   (avisa admin pra conferir)',
  '      uncertainty_reason = "msg ambígua — inferido produto X via EQUIPE"',
  '      confidence  = "medium"  (inferência via contexto, não explícito)',
  '    REGRA RELACIONADA 23 cobre line_changeover (setup); esta é o',
  '    análogo pra production_line corrente. Caso real 29/mai 10:44 AM:',
  '    Ana "S; linha de producao" 40min após Vitor "S: Iniciando linha',
  '    de producao - Potassium" → event da Ana deveria ter product=',
  '    Potassium Iodide BR-2026-0170, cowork=[Vitor], uncertain=true.',
  '    NÃO crie 2 events paralelos por pessoa — UM event por produto+',
  '    batch+tempo, com cowork_with listando os co-trabalhadores.',
  '32. FIM DE EXPEDIENTE = CARIMBO INSTANTÂNEO (bloco 28/mai-noite #32).',
  '    "Máquinas desligadas", "indo embora", "fim do expediente",',
  '    "encerrando o dia", "vou embora", "tchau, até amanhã" = slug',
  '    activity_type="end_of_day" (category=meta, foreground).',
  '    REGRAS:',
  '    (a) Evento INSTANTÂNEO: emita open_event JÁ com ended_at =',
  '        started_at. Não é atividade contínua — é só o carimbo de',
  '        SAÍDA da pessoa.',
  '    (b) O sistema (EventService.upsert) AUTOMATICAMENTE fecha todos',
  '        os foreground LIVE da mesma pessoa no horário do end_of_day,',
  '        com closed_reason="end_of_day". Você NÃO precisa emitir',
  '        close_event manual pra cada fg aberto — apenas o open_event',
  '        do end_of_day.',
  '    (c) Background (is_background=true) NÃO é fechado pelo end_of_day.',
  '        Long_running (multi-dia: Potassium, Chromium etc) continua',
  '        aberto cruzando a noite. Bg single-day fecha pelo próximo',
  '        turno ou pelo safetyAutoClose das 21h NY.',
  '    (d) Cálculo de tempo ativo no dashboard usa o horário do',
  '        end_of_day como FIM da janela de presença da pessoa (não',
  '        NOW). Por isso emitir o end_of_day CERTO é importante:',
  '        sem ele, o card mostra "presente até agora" inflado.',
  '    Caso real 28/mai: Bruno Sarmento 6:46 PM "Fim de expediente —',
  '    máquinas desligadas, indo embora" → ev305 end_of_day instantâneo',
  '    com ended_at=started_at; ev303 (production_line) e ev290 (lunch)',
  '    se ainda estivessem LIVE seriam fechados com closed_reason=end_of_day.',
  '31. PARADA DA LINHA AFETA TODA A EQUIPE (bloco 28/mai-noite #8) —',
  '    line_changeover, machine_downtime e facility_maintenance que',
  '    afeta a linha PARAM o trabalho de TODO MUNDO que estava na linha,',
  '    não só de quem fez a manutenção. Quando emitir um desses 3:',
  '    (a) Adicione cowork_with = TODAS as pessoas que EQUIPE mostra',
  '        ativas na linha de produção naquele momento (regra 23 já faz',
  '        isso pra line_changeover; aplique IGUAL pra machine_downtime',
  '        e facility_maintenance afetando-a-linha).',
  '    (b) Se EQUIPE mostra pessoas na linha que NÃO postaram nada nos',
  '        últimos 30 min E que não entraram em cowork_with, marque',
  '        uncertain=true e uncertainty_reason = "pessoas X, Y na linha',
  '        sem reportar durante parada — pedir esclarecimento". Isso',
  '        sinaliza pro admin (V4 atenção + audit) que precisa perguntar',
  '        via Carolina "o que estão fazendo enquanto a linha está',
  '        parada? Estão ajudando ou em outra coisa?".',
  '    (c) Quem reportar atividade DIFERENTE durante a parada (ex.: foi',
  '        limpar outro setor) abre event próprio com slug correto — não',
  '        entra em cowork. Quem reportar "estou aguardando" entra em',
  '        cowork na parada (default).',
  '    Caso real 28/mai: Vitor + Bruno Sarmento fizeram troca de linha',
  '    manhã. Ana e Simone — sem registro do que fizeram durante. O',
  '    sistema deveria ter emitido uncertain=true pedindo esclarecimento.',
