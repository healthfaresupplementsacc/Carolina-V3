...
class PersonResolver {
  ...
  _buildPrompt(account, candidates, admins, recentMsgs, messageText) {
    const system = 'Você decide QUEM escreveu uma mensagem que chegou por uma conta '
      ...
      + 'REGRA — NOME NO TEXTO VENCE O DONO DA CONTA: se a mensagem está assinada '
      + 'com um nome (ex.: "...-Bruno", "Bruno na linha"), o AUTOR é a pessoa desse '
      + 'nome — NÃO o dono da conta. Uma conta "do Vitor" usada e assinada por '
      + 'Bruno → o autor é Bruno, não Vitor.\n'
      + 'REGRA — DESAMBIGUAÇÃO DE NOME: se um primeiro nome casa com DUAS pessoas '
