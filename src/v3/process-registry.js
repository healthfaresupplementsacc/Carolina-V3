...
const PROCESSES = [
  ...
  },
  {
    ...
    short: 'A cada 5min julga o custo de cada etiqueta nova da Veeqo e COTA antes de aconselhar; cara demais → admin-orin na hora, digest 16:15 NY.',
    detail: 'Bruno 08-28 ("o custo eh uma coisa muito seria... tem como eu saber oq o custo ta acima e oq nao ta antes de imprimir?"). A Veeqo compra etiqueta com data antes do prazo que o cliente pediu (due_date) e o carrier cobra caro. A cada 5min (janela 8h-19h NY) espelha os shipments recentes (48h) em v3.shipment_costs e julga cada etiqueta NOVA contra a mediana móvel 30d da faixa (serviço + faixa de peso; <8 amostras nunca julga; teto absoluto $12 pra <1lb). FASE A do copiloto (09-01): antes de aconselhar, cota as etiquetas novas na Rate Shopping API da Veeqo (mesma chave; cap 25/tick, mais velhas primeiro) e guarda a melhor VÁLIDA (nunca Media Mail/Bound Printed/Library) em quoted_*; o alerta só manda deletar-e-recomprar quando a alternativa mais barata EXISTE (margem $0.25), diz "ja era o melhor preco" quando não existe, e "nao consegui cotar" quando a cotação falhou — falha de cotação nunca derruba o tick. Etiqueta acima do normal → 1 mensagem no admin-orin NA HORA, porque deletar o envio na Veeqo estorna a etiqueta automático (14 dias) MAS pra USPS isso morre quando o SCAN form do dia sai (~tarde): alerta em minutos = dinheiro recuperável. 16:15 NY manda o digest do dia (dedupe audit_log freight_digest; dia normal = 1 linha; com recuperável = "K com opcao mais barata"). Walmart chega com custo 0 (etiqueta deles): contado à parte, nunca em média. Leitura no /api/v3/freight/*, card Frete na página P&P e bloco Copiloto na Central do operador (/api/v3/op-freight/copilot). READ-ONLY na Veeqo (cotar não compra nada), nunca escreve estoque, nunca canal de operador.',
  },
  {
