...
describe('V3 data API — endpoints chamam o repo certo', () => {
  ...
  test('50 endpoints registrados, todos sob /api/v3/data/', () => {
    expect(ENDPOINTS).toHaveLength(50);  // +1 FASE 3: /review-rate; +1 FNSKU: /fnsku
    expect(ENDPOINTS.every((e) => e.path.startsWith('/api/v3/data/'))).toBe(true);
    expect(ENDPOINTS.filter((e) => (e.method || 'get') === 'get')).toHaveLength(26);
