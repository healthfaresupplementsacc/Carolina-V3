...
| R262 | S03.01.04 página P&P (dashboard-v4) card Frete → S02.19 (`useFreight` em `adapters/freight-api.js`, mesmo PIN `getPin` do from-api). Classes próprias `fr-*` de propósito pra não poluir as contagens `.opa-row`/`.card.kpi` do harness da página | VERIFIED | `OtherPages.jsx` FreightCard; `qa-dashboard-operacao.js` (9 asserções novas) |
