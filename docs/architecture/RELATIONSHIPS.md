...
## Vigia de sinais e incidentes (Bruno 08-25)
| R | From → To | Type | Evidence |
|---|---|---|---|
| R253 | S04.36 signal-watchdog → S04.35 signal-registry: o worker NÃO conhece nenhum sinal por dentro, só chama `checkAll(db)` e reage a `stale && in_window`. Sinal novo = um object literal no registro, zero linha no worker | VERIFIED | `signal-watchdog.js tick`; `signal-watchdog.test.js` |
| R254 | S04.36 signal-watchdog → S04.37 incident: todo sinal morto vira `openIncident` com dossiê montado (o que aconteceu, desde quando, o que era esperado, o que se viu, o que quebra downstream, dados crus). O worker não escreve no banco de incidente nem monta texto de Slack sozinho | VERIFIED | `signal-watchdog.js _openFor`; `incident.test.js` |
| R255 | S04.37 incident → `v3.incidents` (migration 082) + `v3.audit_log` (`signal_incident` / `signal_recovered`, dedupe por dia NY, mesmo padrão do S15.28 stock-drift) → admin-orin `C0B36DR5MP1`. NUNCA o canal dos operadores: push morto não é problema que operador resolva | VERIFIED | `incident.js openIncident`; `signal-watchdog.js _opened/_markOpened`; testes de canal |
| R256 | S04.37 incident → Obsidian `Incidentes/` via **filesystem injetado**. No Railway não existe `G:` → degrada guardando o markdown em `incidents.detail->>'dossier_md'`; `flushDossiers({db,fs})` grava depois de uma máquina com o vault e limpa a coluna (idempotente). Escrever direto teria falhado exatamente na hora que importa | VERIFIED | `incident.js openIncident/flushDossiers`; `incident.test.js` (os dois mundos) |
| R257 | S04.38 encap-monitor → S04.37 incident: payload de câmera fresco mas SEM a encapsuladora abre `camera_no_encap_zone` 1×/dia pelo MESMO pipeline (dossiê + admin-orin). É a fresta que produziu o alarme falso de 23/08: as zonas 4 e 6 de `v3.camera_zones` têm a máquina cadastrada e o push do .28 só mandava "Production Line" | VERIFIED | `encap-monitor.js _noEncapZoneIncident/_encapZones`; `encap-monitor.test.js` |
| R258 | S04.38 encap-monitor → `v3.settings machine_state` (S11.01 push do .28): a supressão do alarme agora exige `fresh && matched && moving` da ENCAPSULADORA. `matched:false` e `fresh:false` são ambos "não sei" e NÃO suprimem — antes qualquer máquina em movimento calava a cápsula | VERIFIED | `encap-monitor.js _cameraSaysMoving/tick`; `encap-monitor.test.js` (linha de produção não cala) |
| R259 | S04.39 `GET /api/v3/health/signals` → S04.35 registry + `v3.incidents`. Router NOVO em vez de crescer o `system-health` do `data/router.js` (2106 linhas, proibido crescer); a página Sistema lê os dois lado a lado | VERIFIED | `health/router.js`; `wire.js`; `architecture-drift.test.js` |
| R260 | S04.40 freight-watch → S10.03 Veeqo (`getOrdersPage` status shipped, 48h, read-only) → S08.05.14 `shipment_costs` (upsert idempotente; escrita SÓ via `freight/service.js`) → S10.01 admin-orin (alerta imediato por outlier + digest 16:15 NY, dedupe `audit_log` `freight_digest`). NUNCA canal de operador: recomprar etiqueta é decisão de admin | VERIFIED | `freight-watch.js`; `freight-watch.test.js` (canal, dedupe, janela) |
| R261 | S02.19 `GET /api/v3/freight/*` → S08.05.14 via `freight/service.js` (`summary`/`outliersOf`/`bands`). Router NOVO em vez de crescer o `data/router.js` (2106 linhas, proibido crescer) — mesmo padrão do R259; auth `view_stock` da mesma família do warehouse | VERIFIED | `freight/router.js`; `wire.js`; `architecture-drift.test.js` |
| R262 | S03.01.04 página P&P (dashboard-v4) card Frete → S02.19 (`useFreight` em `adapters/freight-api.js`, mesmo PIN `getPin` do from-api). Classes próprias `fr-*` de propósito pra não poluir as contagens `.opa-row`/`.card.kpi` do harness da página. 09-01: as linhas de outlier ganharam o veredito da cotação (`fr-quote`: "cotei $X svc" verde / "já era o melhor" neutro / "sem cotação" apagado) lido dos campos `quoted_*` do /outliers | VERIFIED | `OtherPages.jsx` FreightCard; `qa-dashboard-operacao.js` (9 asserções novas) |
| R263 | S04.40 freight-watch v3 → Veeqo **Rate Shopping API** (`src/v3/freight/rates-client.js`, POST /shipping/api/v1/rates, mesma `VEEQO_API_KEY` de S10.03, `customer_reference` HF-QUOTE-*) → S08.05.14 colunas `quoted_*` (via `freight/service.saveQuote`; fila = `quoted_at IS NULL`, cap 25/tick). Cotação é CONSELHO: toda falha do client vira null e o tick nunca morre; Fase A nada compra/cancela | VERIFIED | `freight-watch.js` passo 2.5; `rates-client.test.js` (14); `freight-watch.test.js` FASE A (9) |
| R265 | S15.47 `POST /api/v3/warehouse/simple/set` (simple-set.js) → S15.02 StockService (SÓ verbos: storeIn+place pra subir, count absoluto pra descer prateleira/caixa, adjust branch unplaced pra descer a organizar; refs `simpleset:<client_ref>`) + S15.21 locations-repo (prateleira nova via bin_code, qty 0) + StockRequestService `_allocateBoxNumber` (MESMO alocador BX da aprovação, chamado no prototype) + rowsWithVeeqo (resposta com veeqo_total/delta/match). 2+ locais = 409 multi_location → Modo completo. `GET overview` → `simpleProgress(rows)` do mesmo módulo (placar do mutirão) | VERIFIED | `simple-set.js`; `warehouse-simple-set.test.js` (37) |
| R264 | S15.11 Central de P&P (ws.js, bloco "Copiloto de frete" no cartão de etiquetas de envio, mesmo poll de 30 s) → S02.20 `GET /api/v3/op-freight/copilot` → S08.05.14 via `freight/service.copilotSummary` (dia NY). Read-only, ZERO botões na Fase A; auth = a dupla do /op (page token + sessão) num router novo porque op.js não pode crescer | VERIFIED | `ws.js` copilotHtml/loadCopilot; `op-copilot-router.test.js` (6); harness qa-op-ws 82 (3 estados + shots 16-18) |
| R266 | S15.48 Modo simples do hub (WarehousePage vista padrão) → S15.47 `POST simple/set` (absoluto + client_ref uuid; linha repinta da resposta, 409 multi_location vira pulo pro Modo completo) + `GET overview` contrato B (veeqo_total/home_bin/main_box/simple_progress) → S15.24 `GET labels` via `#estoque-etiquetas?bins=&boxes=` (🖨 da linha, mesmo renderizador Code128+QR) → S15.44 `POST count/compute` + `POST weights/product/:id` (⚖ popover, calibrar inline) → S03.01.20 `useAccountPref('estoque.modo')` (a escolha segue a conta). Front NUNCA escreve quantidade fora do simple/set | VERIFIED | `WarehousePage.jsx` simpleCommit/printRow; `qa-dashboard.js` grupos simples+pesar (43) |
| R267 | S03.01 dashboard-v4 `#planejamento` (PlanejamentoPage) → S15.49 `/api/v3/planning/*` (adapter `planning-api.js`, mesmo PIN `getPin`): GET /board (o funil), GET/PUT /plan?date= (lista ORDENADA inteira = persistência do drag), POST/DELETE /plan/item, GET/PUT /notes?date= (autosave debounced), POST /board/boxed (único toque manual do quadro). Auth família warehouse: view_stock lê, manage_stock escreve | VERIFIED | `planning-router.test.js` (auth+CRUD); `qa-dashboard-operacao.js` planejamento 14 asserts |
| R268 | S15.49 planning/model.js → S08.05 tabelas: DERIVA o funil de `ems_activity_cache` (última linha por batch_number, 60d) + `events`/`activity_types` (review · production_line · encapsulation) + `production_counts` (garrafas) + `stock_boxes` (Encaixotado auto) + `production_plan_items` (flags manual_boxed) e GRAVA só `production_plan_items` + `planning_notes` (migração 086); fila viva best-effort via S05.15 `pipeline()` (EMS fora → ems_ok false, quadro segue do cache). Regra crítica: finalized + zero reviews = Esperando revisão | VERIFIED | `planning-model.test.js` (36: classify 7 regras + board) |
