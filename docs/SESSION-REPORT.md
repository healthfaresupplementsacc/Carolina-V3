...
- **22:14 — Phase 2 + STYLE-KIT theme LIVE** (deploy `f0ff7201`, bundle `index-DVe79o27.js`, op sw v40) after Railway degraded-performance retries. Prod smoke: dashboard 200, /op + ws.js 200, warehouse/picklist APIs 200, op stock endpoints gated (401), no pending requests. Everything from the study is now in production; remaining: Bruno registers Locais + Entrada, confirms SKUs, then STOCK_DEDUCT_MODE=live; later retire "(antigo)" pages, days-of-stock column, approval aging nag, roadmap card.
